...
public class PluginMessageListener implements Listener {
   ...
   */
  public PluginMessageListener(AnalyseBungee plugin) {
    this.plugin = plugin;
  }
  ...
  public void onPluginMessage(PluginMessageEvent event) {
    ...
    if (!event.getTag().equals(AnalyseMessaging.CHANNEL)) {
      return;
    }
  ...
  private void handleEventMessage(EventMessage message, String serverName) {
    if (!Analyse.isAvailable()) {
      plugin.getLogger().warning("Received event message but Analyse is not available");
      return;
    }
    ...
    EventBuilder builder = Analyse.trackEvent(message.getEventName())
  ...
  private void handleConversionMessage(ConversionMessage message, String serverName) {
    if (!Analyse.isAvailable()) {
      plugin.getLogger().warning("Received conversion message but Analyse is not available");
      return;
    }
