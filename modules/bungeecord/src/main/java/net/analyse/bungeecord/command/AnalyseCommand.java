...
public class AnalyseCommand extends BaseCommand {
  ...
  public void onTrack(CommandSender sender, String[] args) {
    ...
    Analyse.trackEvent("ingame.buy")
        ...
        .send(success -> {
          plugin.getProxy().getScheduler().runAsync(plugin, () -> {
            if (Boolean.TRUE.equals(success)) {
              send(sender, "&a✓ In-game purchase tracked for &f" + player.getName());
            } else {
  ...
  public void onPurchase(CommandSender sender, String[] args) {
    ...
    if (client == null) {
      send(sender, "&cNo Analyse client available. Make sure a default server is configured.");
      return;
    }
    ...
    client.trackPurchase(request, new AnalyseCallback<PurchaseResponse>() {
      @Override
