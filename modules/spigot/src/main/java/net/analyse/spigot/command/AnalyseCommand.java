package net.analyse.spigot.command;
...
public class AnalyseCommand extends BaseCommand {
  ...
  private void showStatus(CommandSender sender) {
    boolean initialized = plugin.isAnalyseReady();
    boolean apiConnected = Analyse.isConnected();
  ...
  @CommandPermission("analyse.reload")
  public void onReload(CommandSender sender) {
    ...
    if (plugin.getPluginConfig().isValid()) {
      // Reinitialize Analyse with the new config
      plugin.initializeAnalyse();
      send(sender, "&aAnalyse configuration reloaded and initialized successfully.");
    } else {
  ...
  @Syntax("<player_uuid> <purchase_value> <product_name>")
  public void onPurchase(CommandSender sender, String[] args) {
    if (args.length < 3) {
    ...
    plugin.getClient().trackPurchase(request, new AnalyseCallback<PurchaseResponse>() {
      @Override
  ...
  private void showServerInfo(CommandSender sender) {
    ...
    plugin.getClient().getServerInfo(new AnalyseCallback<ServerInfoResponse>() {
      @Override
  ...
  private void showPlayerInfo(CommandSender sender, String playerName) {
    ...
    // Fetch additional data from API
    plugin.getClient().getPlayerInfo(new PlayerInfoRequest(player.getUniqueId()), new AnalyseCallback<PlayerInfoResponse>() {
      @Override
