...
import net.analyse.velocity.addon.VelocityAddonManager;
import net.analyse.velocity.command.AnalyseCommand;
import net.analyse.velocity.config.AnalyseVelocityConfig;
import net.analyse.velocity.listener.ActivityListener;
...
public class AnalyseVelocity implements AnalysePlatform {
  ...
  private void migrateDataFolder() {
    Path oldFolder = dataDirectory.resolveSibling("serverstats");
    if (Files.exists(oldFolder) && Files.isDirectory(oldFolder) && !Files.exists(dataDirectory)) {
      try {
        Files.move(oldFolder, dataDirectory);
        logger.info("Migrated data folder from serverstats to analyse");
      } catch (IOException e) {
        logger.warn("Failed to migrate data folder from serverstats to analyse. Please rename it manually.");
      }
    }
  }
